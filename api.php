...
<main class="container my-5 flex-grow-1">
    <div class="op-kapsayici">
        <header class="text-center mb-5 pb-3">
            <h1 class="display-4 fw-bold text-white mb-3 text-shadow-korsan">DİJİTAL SEYİR DEFTERİ</h1>
        ...
        <ul class="nav nav-pills justify-content-center mb-5 gap-3" id="apiTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link active px-4 py-2 fs-5" id="op-tab" data-bs-toggle="tab"
                    data-bs-target="#onepiece" type="button" role="tab">🏴‍☠️ Grand Line Karakterleri</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link px-4 py-2 fs-5" id="kripto-tab" data-bs-toggle="tab" data-bs-target="#kripto"
                    type="button" role="tab">💰 Kripto Borsası</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link px-4 py-2 fs-5" id="github-tab" data-bs-toggle="tab" data-bs-target="#github"
                    type="button" role="tab">💻 Sistem Reposu</button>
            </li>
        </ul>
        ...
        <div class="tab-content" id="apiTabsContent">
            ...
            <div class="tab-pane fade show active" id="onepiece" role="tabpanel">
                <div class="text-center mb-4">
                    <button id="opBtn" class="btn btn-danger btn-lg fw-bold shadow border-2 border-white px-5">Yeni
                        Karakterler Keşfet ⛵</button>
                </div>
                <div id="opSonuc" class="row g-4 justify-content-center mt-1"></div>
                <div id="opYukleniyor" class="spinner-border text-danger d-none mx-auto mt-5 yukleniyor-spinner d-block"
                    role="status"></div>
            </div>
            ...
            <div class="tab-pane fade" id="kripto" role="tabpanel">
                ...
                <div id="kriptoYukleniyor"
                    class="spinner-border text-info d-none mx-auto mt-5 yukleniyor-spinner d-block" role="status"></div>
            </div>

            <div class="tab-pane fade" id="github" role="tabpanel">
                <div class="text-center mb-4">
                    <button id="githubBtn" class="btn btn-outline-light btn-lg fw-bold px-5">Kaynak Kodları Çek
                        ⚡</button>
                </div>
                <div id="githubSonuc" class="row g-4 justify-content-center mt-1"></div>
                <div id="githubYukleniyor"
                    class="spinner-border text-light d-none mx-auto mt-5 yukleniyor-spinner d-block" role="status">
                </div>
            </div>
...
document.addEventListener("DOMContentLoaded", function() {
        ...
        fetch('https://api.jikan.moe/v4/anime/21/characters')
            ...
            .then(data => {
                ...
                selectedChars.forEach(item => {
                    ...
                    let duzgunIsim = isimParcalari.length > 1 ? isimParcalari[1] + ' ' +
                        isimParcalari[0] : char.name;

                    // Rol bilgisi (Ana karakter mi, Yan karakter mi?)
                    let rolText = item.role === 'Main' ?
                        '<span class="text-danger">Ana Karakter</span>' : 'Yan Karakter';

                    // Karaktere göre rastgele temsili ödül
                    let bounty = item.role === 'Main' ? Math.floor(Math.random() * 900) + 100 : Math
                        .floor(Math.random() * 90) + 1;

                    let kart = `
                    <div class="col-lg-4 col-md-6">
                        <div class="wanted-poster h-100">
                            <div class="wanted-header">WANTED</div>
                            <div class="text-center fw-bold mb-1 wanted-alt-baslik">DEAD OR ALIVE</div>
                            ...
                            <div class="wanted-bounty"><span class="bounty-sembol">฿</span> ${bounty},000,000 -</div>
                        </div>
                    </div>`;
    ...
    function kriptoGetir() {
        ...
        Promise.all(coinler.map(coin => fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${coin}`)
                .then(res => res.json())))
            .then(data => {
                kriptoYukleniyor.classList.add("d-none");
                data.forEach(item => {
                    let isim = item.symbol.replace("USDT", " / USDT");
                    let fiyat = parseFloat(item.price).toLocaleString('en-US', {
